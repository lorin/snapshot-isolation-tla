...
(******************************)
Wr(obj) == LET evt == {h[i] : i \in DOMAIN h} 
            IN {e \in evt : e.op="w" /\ e.obj=obj}

(**************************************)
(* Most recent value written to *obj* *)
(**************************************)
MRW(obj) == 
    LET i == CHOOSE i \in DOMAIN h : 
        /\ h[i].op = "w" 
        /\ h[i].obj = obj
...
SuccessiveReads == LET obj == arg 
                       ...
                       j == CHOOSE j \in 1..Len(h)-1 : NoWrRd(j, obj) 
                   IN (op = "r" /\ \E i \in 1..Len(h)-1 : NoWrRd(i, obj)) => rval = h[j].val
